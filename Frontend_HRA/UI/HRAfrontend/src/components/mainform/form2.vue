<template>
<div>
    <div class="mar0_15">
    <span class="fonweigh700" style="float:right;"><a>X</a></span>
    <h4 class="fonweigh700">Create template</h4>
    <hr style="border-color:#EEEEEE;">
  </div>
<div class="mainfomlayout">
    <ul id="progressbar">
        <li class="active complete clrbluu">Property info</li>
        <li class="active complete clrdarc">Account details</li>
        <li>Rent agreement</li>
    </ul>
    <hr style="border-color:#EEEEEE;">
    <a class="custback" @click="$router.go(-1)"><i class="fas fa-chevron-left"></i>&nbsp;Back</a>
    <div class="row" style="margin-top:10px;font-family: 'Roboto', sans-serif;">
         <form class=" " @submit.prevent="sendData">
         <div class="hrafom1">
        <h4 style="margin: 0px 15px 20px;">Landlord account details</h4>
        <div class="row marzero0">
            <div class="col-sm-6">
                <div class="form-group custrespformgrop">
                        <label class="lblcustm" >Landlord name :</label>
                        <p>
                          <input name="Landlord name" v-model="detail.Landlord_name" v-validate="'required|alpha'" type="text" class="form-control">
                          <span v-show="errors.has('Landlord name')">{{ errors.first('Landlord name') }}</span>
                        </p>
                </div>
            </div>
            <div class="col-sm-6">
                <div class="form-group custrespformgrop">
                        <label class="lblcustm" >Account holder name :</label>
                        <p>
                          <input name="Account holder name" v-model="detail.Account_holder_name" v-validate="'required|alpha'" type="text" class="form-control">
                          <span v-show="errors.has('Account holder name')">{{ errors.first('Account holder name') }}</span>
                        </p>
                    </div>
            </div>
        </div>
        <div class="row marzero0">
            <div class="col-sm-6">
                <div class="form-group custrespformgrop">
                        <label class="lblcustm" >Bank name :</label>
                        <select class="form-control" id="sel1">
                            <option>1</option>
                            <option>2</option>
                            <option>3</option>
                            <option>4</option>
                        </select>
                </div>
            </div>
            <div class="col-sm-6">
                <div class="form-group custrespformgrop">
                        <label class="lblcustm" >IFSC </label>
                        <p>
                          <input name="IFSC" v-model="detail.IFSC" v-validate="'required'" type="text" class="form-control">
                          <span v-show="errors.has('IFSC')">{{ errors.first('IFSC') }}</span>
                        </p>
                    </div>
            </div>
        </div>
        <div class="row marzero0">
            <div class="col-sm-6">
                <div class="form-group custrespformgrop">
                        <label class="lblcustm" >Account number :</label>
                        <p>
                          <input name="Account number" v-model="detail.Account_number" v-validate="'required'" type="text" class="form-control">
                          <span v-show="errors.has('Account number')">{{ errors.first('Account number') }}</span>
                        </p>
                </div>
            </div>
            <div class="col-sm-6">
                <div class="form-group custrespformgrop">
                        <label class="lblcustm" >Retype account number :</label>
                        <p>
                          <input name="Account number" v-validate="'required'" type="text" class="form-control">
                          <span v-show="errors.has('Account number')">{{ errors.first('Account number') }}</span>
                        </p>
                    </div>
            </div>
        </div>
        <div class="row marzero0">
            <div class="col-sm-6">
                <div class="form-group custrespformgrop">
                        <label class="lblcustm" >Landlord city :</label>
                        <p>
                          <input name="Landlord city" v-model="detail.Landlord_city" v-validate="'required'" type="text" class="form-control">
                          <span v-show="errors.has('Landlord city')">{{ errors.first('Landlord city') }}</span>
                        </p>
                </div>
            </div>
            <div class="col-sm-6">
                <div class="form-group custrespformgrop">
                        <label class="lblcustm" >Landlord state :</label>
                        <p>
                          <input name="Landlord state" v-model="detail.Landlord_state"
                          v-validate="'required'" type="text" class="form-control">
                          <span v-show="errors.has('Landlord state')">{{ errors.first('Landlord state') }}</span>
                        </p>
                    </div>
            </div>
        </div>
        <div class="row marzero0">
            <div class="col-sm-6">
                <div class="form-group custrespformgrop">
                        <label class="lblcustm" >Landlord PAN number :</label>
                        <p>
                          <input name="Landlord PAN number" v-model="detail.Landlord_PAN_number" v-validate="'required'" type="text" class="form-control">
                          <span v-show="errors.has('Landlord PAN number')">{{ errors.first('Landlord PAN number') }}</span>
                        </p>
                </div>
            </div>
        </div>
        <div class="row marzero0">
            <div class="col-sm-6">
                <div class="form-group custrespformgrop">
                        <label class="lblcustm" >Upload landlord PAN :</label>
                        <div class="file-drop-area">
                            <!-- <span class="fake-btn">Choose files</span> -->
                            <span class="file-msg js-set-number" style="color:#808080;font-size:16px">Click or Drop files <span style="color:#0071bc;">here</span></span>
                            <input class="file-input" type="file" multiple>
                        </div>
                        <small>jpg/png/pdf files with a size less than 500kb</small>
                </div>
            </div>
        </div>
        <div class="row marzero0">
            <label class="lblcustm" style="margin:0px 15px;" >Maintenance Account Details :</label>
            <div class="col-sm-12">
                <div class="form-group custrespformgrop">
                        <label class="lblcustm" >Same as landlord account details?</label>
                        <input type="radio" id="option1" name="fooBar" value="foo">
                        <label class="radiolabel" for="option1"><span><span></span></span>Yes</label>

                        <input type="radio" id="option2" name="fooBar" value="bar">
                        <label class="radiolabel" for="option2"><span><span></span></span>No</label>
                </div>
            </div>
        </div>

        <div class="row marzero0" style="margin-bottom:20px;">
            <div class="maxbtnwid ">
            <router-link :to="{ name: 'form3' }"><div class="col-xs-6"><button type="button" class="btn btn-info custbtnn" @click="sendData">Save and Next</button></div></router-link>
            </div>
        </div>
    </div>
  </form>`
    </div>
     <hr style="border-color:#EEEEEE;">
    <p style="font-size:12px;color:#B0BEC5;text-align:center;">© Copyright 2015-2018 Paymatrix. All Rights Reserved.</p>
</div>

</div>
</template>

<script>
export default {
  data() {
    return {
    detail:{
    Landlord_name:' ',
    Account_holder_name:' ',
    Bank_name:' ',
    IFSC:' ',
    Account_number:' ',
    Landlord_city:' ',
    Landlord_state:' ',
    Landlord_PAN_number:' ',
    }
    }
  },
  methods: {
   sendData() {
    this.$store.Landlord_detail=this.detail;
  }
}

}
</script>

<!-- Add "scoped" attribute to limit CSS to this component only -->
<style>
.custback{
    color: #546E7A;
    font-weight: 600;
}
.lblcustm{
    font-weight: 400;
    color: #536771;
}
.clrlitegry{
    color: #B0BEC5;
}

.mar05{
    margin: 0px 5px;
}
.cuspointer{
    cursor: pointer;
}
.modal{
        background: rgba(0,0,0,0.4);
}
.boxul{
    padding-left: 10px;
    list-style: none;
}
/* custom radio button */
/* TODO change the cursor */
/* Hide default checkboxes */
input[type=radio]{
  display: none;
}

/* Style custom radio button shell */
input[type=radio] + label > span {
  display: inline-block;
  width: 15px;
  height: 15px;
  margin: 3px 6px 3px 3px;
  border: 1px solid #546E7A;
  border-radius: 8px;
  vertical-align: bottom;
}

/* If you want cursor change on hover */
/* input[type=radio] + label > span:hover {
  cursor: pointer;
} */

/* Style custom selected radio button bullet */
input[type=radio]:checked + label > span > span {
  display: block;
  width: 7px;
  height: 7px;
  margin: 3px;
  border: 1px solid #3284EF;
  border-radius: 5px;
  background: #3284EF;
}

.radiolabel {
    display: inline-block;
    font-size: 14px;
    color: #363738;
    margin: 10px 10px;
}

.mainfieldheading{
    text-align: center;
     margin: 20px 0px 0px;
}
.clrdrcgray{
    color: #546E7A
}
</style>
